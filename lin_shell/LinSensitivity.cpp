...
LinSensitivity::LinSensitivity(FEAMesh &feaMesh_, std::vector<GptsCompl> &gaussComplSens_):
                            gaussComplSens(gaussComplSens_), feaMesh(feaMesh_)
{
...
std::vector<double> LinSensitivity::get_bptsCompl(MatrixXd &BptsCoords, double radius)
{
    ...
    for (unsigned int nn = 0; nn < nBpts; nn ++)
    {
        ...
        BptsSensitivities[nn] = ComputeBoundaryPointSensitivity2D(BptsCoord[nn], radius, WeightFlag, 0.01); // TOFIX: temporarly, only 2D (i.e., flat) config is considered
    }
