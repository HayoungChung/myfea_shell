...
Vector2d CoreElement::get_StressStrain(FilteredP & p_d, Matrix<double, 6, 1> & FNM_r, double r, double s, Matrix<double, 1, 6> & stress, Matrix<double, 1, 6> & strain, Matrix3d & globalXY0){ // computes stress and strain
...
void CoreElement::get_gradStress(Matrix<double, 1, 6> & stress, double r, double s, Matrix<double, 1, 6> & GradStress){ // computes stress and strain
    Vector3d x_ = xycoord.row(0), y_ = xycoord.row(1);
    Matrix3d Amat = Mater_e.Amat, Dmat = Mater_e.Dmat, Bmat = Mater_e.Bmat;
...
void CoreElement::get_filteredmat(FilteredP & p_d, Matrix<double, 6, 1> & FNM_r, std::vector<double> ri, std::vector<double> si, std::vector<double> wi){
    Matrix<double,9,1> u_m = p_d.membrane, u_b = p_d.bending;
    Vector3d Nph = FNM_r.topRows(3), Mph = FNM_r.bottomRows(3);
    Vector3d x_ = xycoord.row(0), y_ = xycoord.row(1);
    Matrix3d Amat = Mater_e.Amat, Dmat = Mater_e.Dmat, Bmat = Mater_e.Bmat;
