...
EICR_SHELL::EICR_SHELL(FEAMesh & feaMesh_, int ngp) : feaMesh(feaMesh_){
    ...
    ri.reserve(ngp); si.reserve(ngp); wi.reserve(ngp);
...
void EICR_SHELL::assembly(MatrixXd & GU_u0_, VectorXd & GU_Rv0_, std::vector<Material_ABD> & material_, struct Force & force_){//, bool issens){
