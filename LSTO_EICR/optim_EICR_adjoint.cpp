// this code compares nonlinear sensitivity with linear one, at small deformation (f = 0.001)
// ./../lin_shell/main_lin
...
int main(int argc, char *argv[])
{
	...
	for (unsigned int dd = 0; dd < nNODE; ++dd)
	{ // to 2D
	...
	// feaMesh.set_Force(1, tipnode1, -0.001, force.fix);
	...
	*/
	...
	while (n_iterations < maxIter)
	{
		...
		for (unsigned int ee = 0; ee < nELEM; ee++)
		{
			if (lsmMesh.elements[ee].area <= 1e-2)
				feaMesh.areafraction[ee] = 0;
		...
		MatrixXd p_Adjoint6 = MatrixXd::Zero(nNODE, 6);
		// output: GU_u, GU_Rv
		std::vector<GptsCompl> gaussSens = f_nlgeom(material, force, feaMesh, option,
													GU_u, GU_Rv, p_Adjoint6);

		snprintf(fname, sizeof(char) * 32, "step_%i.vtk", n_iterations);
		feaMesh.to_vtk(GU_u, fname); //verified

		snprintf(fname, sizeof(char) * 32, "Adjoint_step_%i.vtk", n_iterations);
		feaMesh.to_vtk(p_Adjoint6, fname); //verified

		snprintf(fname, sizeof(char) * 32, "disp_%i.txt", n_iterations);
		std::ofstream dipfile(fname);
		dipfile << GU_u << endl;

		snprintf(fname, sizeof(char) * 32, "rot_%i.txt", n_iterations);
		dipfile.open(fname);
		dipfile << GU_R << endl;
		dipfile.close();

		snprintf(fname, sizeof(char) * 32, "pAdj_%i.txt", n_iterations);
		dipfile.open(fname);
		dipfile << p_Adjoint6 << endl;
		dipfile.close();

		compliance = (GU_u.cwiseProduct(force.fix.leftCols(3))).sum();

		/*
		// Old ver of computing sensitivity =================================
		double p_Adjoint = f_nlgeom(material, force, feaMesh, option,
							 GU_u, GU_Rv); 
		compliance = (GU_u.cwiseProduct(force.fix.leftCols(3))).sum();
		...
		snprintf(fname, sizeof(char) * 32, "step_%i.gptSens.txt", n_iterations);
		std::ofstream gsfile(fname);
		for (unsigned int ii = 0; ii < sens.gptsSens.size(); ii++)
		{
			gsfile << sens.gptsSens[ii].x << " " << sens.gptsSens[ii].y
			<< " " << sens.gptsSens[ii].z << " " << sens.gptsSens[ii].sens << std::endl;
		}
		gsfile.close();
		...
		*/
		...
		snprintf(fname, sizeof(char) * 32, "step_%i.gptSens.txt", n_iterations);
		std::ofstream gsfile(fname);
		for (unsigned int ii = 0; ii < gaussSens.size(); ii++)
		{
			gsfile << gaussSens[ii].x << " " << gaussSens[ii].y
				   << " " << gaussSens[ii].z << " " << gaussSens[ii].sens << std::endl;
		}
		gsfile.close();

		SensitivityAnalysis m2doLeastsquare(feaMesh, gaussSens);
		// WIP ==================================================================
		...
		for (int i = 0; i < boundary.points.size(); i++)
		{
			...
			bsfile << bPoint[0] << " " << bPoint[1] << " "
				   << m2doLeastsquare.boundarySens[i] << std::endl;
		}
		bsfile.close();
